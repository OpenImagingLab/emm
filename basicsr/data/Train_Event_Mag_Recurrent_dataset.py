...
class EventMagRecurrentDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(EventMagRecurrentDataset, self).__init__()
        self.opt = opt
        ...
        for video in video_list:
            ## frames
            frames  = sorted(recursive_glob(rootdir=os.path.join(self.dataroot, self.split, video, 'gt'), suffix='.png')+recursive_glob(rootdir=os.path.join(self.dataroot, self.split, video, 'gt'), suffix='.jpg'))  # all sharp frames in one video
            frames_step = sorted(recursive_glob(rootdir=os.path.join(self.dataroot, self.split, video, 'step'), suffix='.png')+recursive_glob(rootdir=os.path.join(self.dataroot, self.split, video, 'step'), suffix='.jpg'))
    ...
    def __getitem__(self, index):
        ...
        input_idx = [0,-1]
        gt_idx = list(range(1, self.setLength))
        image_paths = [step_image_paths[idx] for idx in input_idx]
        gt_paths = [all_image_paths[idx] for idx in gt_idx]

        # ipdb.set_trace()
        assert len(event_paths) == len(gt_paths), 'The length error ' + step_image_paths[0] # cyt
        ...
        voxels_list = img_results[num_lq+num_gt:] # 1,60,256,256
        ## Norm voxel
        if self.norm_voxel:
        ...
        alpha = int(seq.split('_')[1])
        # pdb.set_trace()
        
        if self.split == 'train':
            return {'lq': img_lqs, 'gt': img_gts, 'voxel': voxels, 'seq': seq, 'origin_index': origin_index, 'alpha':alpha}
        else:
            return {'lq': img_lqs, 'gt': img_gts, 'voxel': voxels, 'seq': seq, 'origin_index': origin_index, 'alpha':alpha}
