...
def events_to_voxel_grid(events, num_bins, width, height, return_format='CHW'):
    ...
    assert(len(events) == num_bins)
    assert(num_bins > 0)
    assert(width > 0)
    assert(height > 0)

    voxel_grid_pos = np.zeros((num_bins, height, width), np.float32).ravel()
    voxel_grid_neg = np.zeros((num_bins, height, width), np.float32).ravel()
    ...
    for i,event in enumerate(events):
        ...
        tis = np.ones(event[:, 0].shape).astype(np.int)*i
        xs = event[:, 1].astype(np.int)
        ys = event[:, 2].astype(np.int)
        pols = event[:, 3].astype(np.int)
        pols[pols == 0] = -1  # polarity should be +1 / -1

        pols_pos = pols.copy()
        pols_pos[pols_pos == -1] = 0

        pols_neg = pols.copy()
        pols_neg[pols_neg == 1] = 0
        ...
        np.add.at(voxel_grid_pos, xs + ys * width + tis * width * height, pols_pos)
        np.add.at(voxel_grid_neg, xs + ys * width + tis * width * height, pols_neg)

        # valid_indices = (tis + 1) < num_bins
        # np.add.at(voxel_grid, xs[valid_indices] + ys[valid_indices] * width
        #         + (tis[valid_indices] + 1) * width * height, vals_right[valid_indices])

    voxel_grid_pos = np.reshape(voxel_grid_pos, (num_bins, height, width))
    voxel_grid_neg = np.reshape(voxel_grid_neg, (num_bins, height, width))

    voxel_grid_neg = np.absolute(voxel_grid_neg)

    voxel_grid = np.stack((voxel_grid_pos,voxel_grid_neg),axis=1)

    voxel_grid = np.reshape(voxel_grid,(num_bins*2,voxel_grid.shape[2],voxel_grid.shape[3])) # 60,256,256
