...
class EventBasedMotionMagModel(BaseModel):
    """Base event-based motion magnification model for training."""

    def __init__(self, opt):
        super(EventBasedMotionMagModel, self).__init__(opt)
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            for frame_idx in range(visuals['result'].size(1)):
                ...
                if save_img:
                    ...
                    # plt.imshow(event_img_pos, vmin=event_img_pos.min(), vmax=event_img_pos.max())
                    # plt.colorbar()
                    # # plt.subplot(1, 2, 1); plt.imshow(event_img_pos, vmin=-5, vmax=5)
                    # # plt.subplot(1, 2, 2); plt.imshow(event_img_neg, vmin=-5, vmax=5)
                    # plt.savefig(save_event_path)
                    # plt.close()
