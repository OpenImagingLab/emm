...
class EvsMotionMagFilterNet(FinalDecoderRecurrentUNet):
    ...
    def forward(self, x, event ,alpha = 40):
        ...
        # ipdb.set_trace()
        if x.dim()==5:
            x = rearrange(x, 'b t c h w -> (b t) c h w') # sharp [2, 3, 360, 640]
        b, t, num_bins, h, w = event.size()
        event = rearrange(event, 'b t c h w -> (b t) c h w') # [4, 2, 360, 640]
        # print("event", b,t,num_bins,h,w)
        ...
        for frame_idx in range(0,t):
            ...
            if dM_state is None:
                dM_state = torch.zeros(dM.shape + (t,),device=dM.device)
            dM_state[:, :, :, :, frame_idx] = dM
        
        if self.enable_filter:        
            ## TODO: add temporal filter to dM_state
            # hyper param
            n_filter_tap = t
            ...
            filter_b = torch.from_numpy(filter_b).to(dM.device)
            filter_fft = torch.fft.fft(torch.fft.ifftshift(filter_b),
                                        n=dM_state.shape[-1])
            # ipdb.set_trace()
            # Filtering
            for i in range(dM_state.shape[1]):
                x_fft = torch.fft.fft(dM_state[:, i, :, :], axis=-1)
                x_fft *= filter_fft[np.newaxis, np.newaxis, np.newaxis, :]
                dM_state[:, i, :, :] = torch.fft.ifft(x_fft)
            
        for frame_idx in range(0,t):
            
            # --------Manupulator--------
            # ipdb.set_trace()
            M_mag = M[:,0,...] + alpha * dM_state[:, :, :, :, frame_idx] 
...
if __name__ == '__main__':
    ...
    model = TestTuneBidirectionAttenfusion_v4(img_chn=3, ev_chn=2)
    device = 'cuda'
