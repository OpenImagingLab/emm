...
class TwoSharpImageEventRecurrentRestorationModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # print(current_iter,self.gt.shape)
        if current_iter % 500 ==0:
            self.writer.add_scalar("Loss/train",loss_dict['l_pix'],current_iter)
            
            cv2.imwrite("./debug/"+str(current_iter)+"gt.jpg",np.array((self.gt[0,1,...]*255).permute(1,2,0).cpu()))
            cv2.imwrite("./debug/"+str(current_iter)+"pred.jpg",((pred[0,1,...]*255).permute(1,2,0).cpu().detach().numpy()))
            cv2.imwrite("./debug/"+str(current_iter)+"end0.jpg",((self.lq[0,0,...]*255).permute(1,2,0).cpu().detach().numpy()))
            cv2.imwrite("./debug/"+str(current_iter)+"end1.jpg",((self.lq[0,1,...]*255).permute(1,2,0).cpu().detach().numpy()))
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, #here
        ...
        for idx, val_data in enumerate(dataloader):
            # val_data:dict_keys(['lq', 'gt', 'voxel', 'seq', 'origin_index'])
            # print('voxel',val_data["voxel"].shape)
            # val_data["voxel"][0,1,0,:,:] = val_data["voxel"][0,0,0,:,:]
            # val_data["voxel"][0,1,1,:,:] = val_data["voxel"][0,0,1,:,:]
            # val_data["voxel"][0,2,0,:,:] = val_data["voxel"][0,0,0,:,:]
            # val_data["voxel"][0,2,1,:,:] = val_data["voxel"][0,0,1,:,:]
            # print('voxel',val_data["voxel"].shape)
